...
/**
 * <head> de una pagina de tema de "Un ensayo mas" (1998). Calcado de
 * seo/track.js, pero sin inAlbum (no es parte del Demo'98) y con el
 * breadcrumb pasando por /ensayos en vez de #letras.
 */
const SeoEnsayoTrack = ({ track }) => {
  ...
  const title = `${track.title} — ${copy.ensayos.tapeTitle} | HIJOS DEL SOL`;
  const description = copy.ensayos.trackDescription(track.title);
  const imageAlt = copy.ensayos.coverAlt(track.title);
  const lyricsLang = track.lang === "ES" ? "es" : "en";
  const videoName = `HIJOS DEL SOL — ${track.title} (Un ensayo más, 1998)`;
      ...
      },
      {
        ...
        itemListElement: [
          ...
          },
          {
            "@type": "ListItem",
            position: 2,
            name: copy.ensayos.title,
            item: localeUrl(locale, "/ensayos"),
          },
